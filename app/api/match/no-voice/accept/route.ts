...
// POST /api/match/no-voice/accept
export async function POST(request: Request) {
  ...
  const { error } = await admin.rpc("accept_partner_no_voice", {
    p_user_id: user.id,
    p_match_id: matchId,
  });
  ...
  if (error) {
    ...
    if (error.message.includes("no_voice_accept_not_allowed")) {
      return Response.json({ ok: false, errorKey: "no_voice_accept_not_allowed" }, { status: 400 });
    }
    return Response.json({ ok: false, errorKey: "accept_failed" }, { status: 500 });
  }
